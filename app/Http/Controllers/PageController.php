...
class PageController extends Controller
{
   ...
   public function adminEmailChecker(String $request)
   {
    ...
    public function adminSaver(Person $admin, Request $request)
    {
        ...
        if (self::adminEmailChecker($admin_email) || self::adminUnameChecker($admin_username)) {
    ...
    public function registerAdmin(Request $request)
    {
        ...
        return self::adminSaver($newadmin, $request);
    }
    ...
    public function saveAdmin(Request $request)
    {
        ...
        if ($keyword != $admin_username) {
            if (self::adminUnameChecker($admin_username) || self::adminEmailChecker($admin_email)) {
                return redirect()->route('editadmin', ['id' => $keyword])->withErrors('An admin with that username or email has already exists!');
            }
            $oldadmin = Person::where('username', '=', $keyword)->first();
            $oldadmin->delete();
            $newadmin = new Person();
            self::adminSaver($newadmin, $request);
        } elseif ($keyword == $admin_username) {
            if (self::adminEmailChecker($admin_email) && (bool) $admin_username == Person::where('email', '=', $request)->first()) {
                return redirect()->route('editadmin', ['id' => $keyword])->withErrors('An admin with that email has already exists!');
            }
            $oldadmin = Person::where('username', '=', $keyword)->first();
            self::adminSaver($oldadmin, $request);
        }
    ...
    public function saveUser(Request $request)
    {
        ...
        if ($keyword != $user_username) {
            ...
            if (!is_null($unamequery) || !is_null($emailquery)) {
                return redirect()->route('edituser', ['id' => $keyword])->withErrors('An user with that username or email has already exists!');
            }
            ...
            $newuser->save();
        } elseif ($keyword == $user_username) {
            $emailcheck = $request->input('email');
            $emailquery = Person::where('email', '=', $emailcheck)->first();

            if (!is_null($emailquery) && (bool) $user_username == Person::where('email', '=', $request)->first()) {
                return redirect()->route('edituser', ['id' => $keyword])->withErrors('An user with that email has already exists!');
            }
    ...
    public function saveKategori(Request $request)
    {
        ...
        if ($keyword != $user_username) {
            ...
            if (!is_null($unamequery)) {
                return redirect()->route('editkategori', ['nama' => $keyword])->withErrors('An user with that username or email has already exists!');
            }
    ...
    public function saveProduct(Request $request)
    {
        ...
        if ($keyword != $user_username) {
            ...
            if (!is_null($unamequery)) {
                return redirect()->route('editproduct', ['nama' => $keyword])->withErrors('A product with that username or email has already exists!');
            }
